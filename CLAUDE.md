...
## 코드 수정 절대 원칙
- **코드를 수정할 때는 반드시 기존보다 1가지 이상 나아져야 한다.** 수정 후 기존에 동작하던 기능이 하나라도 안 되면 절대 금지.
- **기능 퇴보 절대 금지**: 카카오톡 공유가 되다가 안 되거나, 이미지 저장이 되다가 안 되거나, 모바일에서 되다가 안 되는 등 기존 동작하던 기능이 깨지는 수정은 절대 하지 않는다.
- 수정 전 반드시: 기존에 동작하는 기능 목록을 체크리스트로 만들고, 수정 후 전부 동작하는지 확인한 뒤에만 commit할 것.
