...
const TIER_BADGE_STYLE: Record<number, string> = {
  ...
  1: 'bg-amber-50 text-amber-600 ring-amber-200',
...
export default function ProgramCard({ program, userTier }: Props) {
  ...
  if (canAccess) {
    return (
      <Link href={`/programs/${program.id}`}>
        <div className={`group bg-white rounded-2xl p-3.5 sm:p-5 border ${program.badge ? 'border-orange-200 ring-1 ring-orange-100' : 'border-gray-100'} card-hover h-full relative overflow-hidden active:scale-[0.98] transition-transform`}>
          {program.badge && (
  ...
  return (
    <div
      className="group/locked bg-white/60 rounded-2xl p-3.5 sm:p-5 border border-gray-100 h-full relative overflow-hidden cursor-pointer active:scale-[0.98] transition-transform"
