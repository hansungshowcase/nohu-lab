        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                setOpen(false)
                const cat = new URL(item.href, window.location.origin).searchParams.get('category') || ''
                window.dispatchEvent(new CustomEvent('category-change', { detail: cat }))
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2.5 rounded-xl shadow-lg shadow-amber-900/5 border border-amber-200/40 transition-all" style={{ background: 'rgba(253,248,240,0.95)', backdropFilter: 'blur(8px)' }}
      >
