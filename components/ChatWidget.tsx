...
export default function ChatWidget({ user }: { user: User | null }) {
  ...
  return (
    <>
      {/* 채팅창 */}
      {open && (
        <div className="fixed sm:bottom-20 sm:right-6 sm:w-[340px] sm:h-[480px] sm:max-h-[calc(100dvh-8rem)] sm:rounded-2xl inset-0 sm:inset-auto w-full h-full bg-white shadow-2xl shadow-black/15 border-0 sm:border border-gray-200 flex flex-col z-50 animate-[slideUp_0.15s_ease-out] safe-bottom">
          {/* 헤더 */}
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5 min-h-0">
            {!loaded ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 gap-2">
                ...
                <p className="text-[13px]">관리자에게 메시지를 보내보세요!</p>
              </div>
            ) : (
              messages.map((msg) => {
          ...
          <form onSubmit={handleSend} className="shrink-0 px-3 py-2.5 border-t border-gray-100">
            <div className="flex gap-2">
              <input
                ...
                placeholder="메시지 입력..."
                maxLength={1000}
                className="flex-1 px-3 py-2 min-h-[44px] rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-base focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              />
              <button
                type="submit"
                disabled={!input.trim() || sending}
                className="p-3 min-w-[44px] min-h-[44px] bg-orange-600 hover:bg-orange-700 disabled:bg-gray-300 text-white rounded-xl transition shrink-0 flex items-center justify-center"
              >
